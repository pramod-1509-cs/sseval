...
char label[50], opcode[50], operand[50], symbol[50], value[10], mnemonic[50], operand_address[5], opcode_address[10], locctr[10], object_code[20], text_record[100];
int length = 0;
char integer[20];
int cur_length = 0;
int is_last = 1;
...
void get_length()
{  int l;
    FILE *fp4 = fopen("program_length.txt", "r");
    if(fp4== NULL)
...
void pass2()
{
    ...
    int start;
    char line[100];
    size_t len = 100 * sizeof(char); // size_t is an unsigned integer data type used for storing size
    ...
    while ((fgets(&line, &len, fp1)) != NULL)
    {
        ...
        if((!strcmp(label, " ")==0) || (!strcmp(opcode, " ")==0) || (!strcmp(operand, " ")==0))
        {
            ...
            {
                int prev_loc =0;
                cur_length = (int)strtol( operand, NULL, 16) - starting_address;
                fprintf(fp3, "%02x%s\n",, text_record);
                strcpy(text_record, "^");
                strcat(text_record, object_code);
                ...
                fprintf(fp3, "%06x^", starting_address);
                is_last = 1;
                prev_loc = (int)strtol(locctr, NULL, 16);
...
void show_output()
{
    ...
    FILE *fp9 = fopen("object_program.txt", "r");
    FILE *fp10 = fopen("program_length.txt", "r");
    char locctr[50];
    ...
    if(fp9== NULL)
       // printf("Error Opening object_program.txt\n");
   ...
   // while ((fgets(&line1, &len1, fp9)) != NULL)
        //printf("%s", line1);
